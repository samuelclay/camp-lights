...
#define CHYRON_WIDTH 24
#define COLOR_DROPOFF 2
#define SPEED 20
#endif
...
#define CHYRON_WIDTH 12
#define COLOR_DROPOFF 3
#define SPEED 30
#endif
...
#define GROUP_COUNT 3
#define CHYRON_WIDTH 32
#define COLOR_DROPOFF 16
#define SPEED 40
#endif
...
void raindrop(unsigned long color, byte wait) {
  ...
  const byte weight = COLOR_DROPOFF;  
  ...
  for (int i=0; i<=LED_COUNT-1; i++) {
    ...
    for (int j=1; j<CHYRON_WIDTH; j++) {
      byte redWJ = red/(j/weight);
      byte greenWJ = green/(j/weight);
      byte blueWJ = blue/(j/weight);
      if (i-j >= 0) {
        for (int g=0; g<GROUP_COUNT; g++) {
